...
for s = 1:nSets
   ...
   if isfield(data(s),{'contrast'})
      csf(s) = condParser(log(data(s).contrast),sfs);
      csf(s).perf = repmat(exp(csf(s).perf)',1,2);
   else
      csf(s).perf = 0;
   end
...
avgPerf = squeeze(median(setPerf,1));
...
for e = 1:numel(eccVal)
   ...
   for c = 1:numel(cues.label)
      ...
      % pretty up figures
      set(gca,'XTick',2.^(-2:3),'XLim',[0.3 20],'YLim',[0.5 1],'box','off','TickDir','out','YTick',0:0.25:1,'XScale','log');
