...
% remove psi method structure from stimulus variable and then save
pm = stimulus.taskParams.psiM;
stimulus.taskParams.psiM = [];
save([myscreen.datadir,'psi_method.mat'],'pm','stimulus');

% end task
endTask(myscreen,task);

% put psi method structure back into stimulus variable
stimulus.taskParams.psiM = pm;
...
thisPsi = stimulus.taskParams.psiM(thisEcc,thisSF);

% transform contrast from log to linear units
c = min(10^thisPsi.xCurrent,1);
...
if stimulus.taskParams.verifyThresh
   ...
   if task.thistrial.ecc==0
      distLoc = distLoc(:,~ismember(distLoc(2,:),task.thistrial.ecc));
   end
   ...
   distSF = repmat(find(stimulus.taskParams.sfs==task.thistrial.sfs),1,numel(distEccIdx));
   c = 10.^stimulus.thresh(find(stimulus.taskParams.sfs==task.thistrial.sfs),distEccIdx);
...
if length(resp)>1
   ...
   stimulus.contrast(task.trialnum) = thisPsi.xCurrent;

   % update psi method
   if ~stimulus.taskParams.verifyThresh
      stimulus.taskParams.psiM(thisEcc,thisSF) = PAL_AMPM_updatePM(thisPsi,correct);
   end
