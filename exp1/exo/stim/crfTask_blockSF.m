...
[~,thisSF] = ismember(task.thistrial.sfs,stimulus.taskParams.all_sfs);
[~,thisEcc] = ismember(task.thistrial.ecc,task.parameter.ecc);

% get threshold estimate for current condition
thresh = stimulus.thresh(thisSF,thisEcc);
...
c = 10^allContrasts(task.thistrial.contrast);

% store contrast level
stimulus.contrastVal(task.trialnum) = c;
...
if stimulus.brokenTrial.trialIdx(task.trialnum)
    ...
    stimulus.contrastVal(task.trialnum) = nan;
