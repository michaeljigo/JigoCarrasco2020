...
function csfTask_stairHL(myscreen)
global stimulus
...
whichRep = arrayfun(@(x) length(x.allLevels)==stimulus.stairParams.nTrials,squeeze(stimulus.stair(:,:,:,:,end)));
if all(whichRep(:)==1)
   whichRep = stimulus.stairParams.stairRep+1;
end
if whichRep>size(stimulus.stair,5)
   newStair = initStair_stairHL(myscreen.subj,stimulus.taskParams,stimulus.stairParams,1);
   ...
   switch task.thistrial.thisseg
      case 1 % mandatory fixation period
         ...
         if stimulus.eyeParams.recalibrate
            ...
            mglClearScreen; drawFixAndPlaceholders; mglFlush
            mglClearScreen; drawFixAndPlaceholders; mglFlush
            mglWaitSecs(1);
         end
